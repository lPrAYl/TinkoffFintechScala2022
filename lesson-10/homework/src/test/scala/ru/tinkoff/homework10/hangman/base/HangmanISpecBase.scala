package ru.tinkoff.homework10.hangman.base
...
trait HangmanISpecBase extends AsyncFunSuite with BeforeAndAfterAll {
  ...
  test("Player can win hangman game") {
    for {
      ...
      val expectedGameState = initialGame.copy(
        state = State(playerName, guesses.toSet, realWord),
  ...
  test("Player can lost hangman game") {
    for {
      ...
      realWord = realGame.state.word
      guesses = ('a' to 'z')
        .filterNot(c => realWord.contains(c))
        .take(10)
        .toList
      gameHistory <- makeGuesses(initialGame.id, guesses)
      finishedGame <- getGame(initialGame.id)
      ...
      assert(expectedStates == stateHistory)
      val expectedGameState = initialGame.copy(
        state = initialGame.state.copy(guesses = guesses.toSet),
  ...
  private def recreateStates(guesses: Seq[Char], state: State) =
    guesses
      ...
      .map(guesses =>
        state.word.map { c => if (guesses.contains(c)) c else '*' } -> guesses
      )
      .map { case (word, guesses) =>
        state.copy(word = word, guesses = guesses.toSet)
      }
      .tail
  ...
  private def makeGuess(gameId: Long, guess: Char): Future[Game] =
    SimpleHttpClient.post[Game](
      Uri(s"http://localhost:8080/game/$gameId/guess?letter=$guess")
    )
  ...
  private def startGame(): Future[Game] = {
    SimpleHttpClient.post[Game](
      Uri(s"http://localhost:8080/game?userName=$playerName")
    )
  }
...
object SimpleHttpClient extends LazyLogging {
  def post[U: FromResponseUnmarshaller](
      uri: Uri
  )(implicit ac: ActorSystem, ec: ExecutionContext): Future[U] = {
    logRequest(uri, POST)
    Http()
      .singleRequest(HttpRequest(POST, uri = uri))
      .flatMap(response => Unmarshal(response).to[U])
  }

  def get[U: FromResponseUnmarshaller](
      uri: Uri
  )(implicit ac: ActorSystem, ec: ExecutionContext): Future[U] = {
    logRequest(uri, GET)
    Http()
      .singleRequest(HttpRequest(GET, uri = uri))
