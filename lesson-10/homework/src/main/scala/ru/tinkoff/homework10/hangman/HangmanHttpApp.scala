...
import ru.tinkoff.homework10.hangman.api.{
  AdminApi,
  GameApi,
  HangmanExceptionHandler
}
import ru.tinkoff.homework10.hangman.logic.{
  DictionaryService,
  GameService,
  GameServiceImpl,
  PlayerGameService,
  PlayerGameServiceImpl
}
import ru.tinkoff.homework10.hangman.storage.{GameStorage, ImMemoryGameStorage}
...
case class HangmanGame()(implicit ac: ActorSystem, ec: ExecutionContext)
    extends LazyLogging {

  private val dictionaryService = new DictionaryService
  private val storage: GameStorage = new ImMemoryGameStorage
  private val gameService: GameService =
    new GameServiceImpl(dictionaryService, storage)
  private val playerGameService: PlayerGameService = new PlayerGameServiceImpl(
    gameService
  )
